    ...
    <div class="project-page">
      <section class="dashboard-header pt-5">
          ...
          <div class="w-full lg:max-w-full lg:flex mb-4">
            ...
            >
              <div>
                ...
                <p class="md:text-4xl text-gray-700 text-base">Rp. {{ Intl.NumberFormat().format(campaign.data.goal_amount) }}</p>
              </div>
            </div>
          </div>
        </div>
        <div class="flex justify-between items-center">
          ...
          <input type="file" ref="file" @change="selectFile" class="border p-1 overflow-hidden rounded">
            <button
              @click="upload"
        ...
        <div class="grid grid-cols-4 gap-4 -mx-2">
          <div v-for="image in campaign.data.images" :key="image.image" class="relative w-1/4 bg-white p-2 border border-gray-400 rounded">
            <figure class="item-thumbnail">
              <img :src="$axios.defaults.baseURL+ '/' + image.image" alt="" class="rounded w-full" />
            </figure>
          </div>
        </div>
            ...
            >
              <div>
                ...
                <p class="text-sm text-gray-600 flex items-center mb-2">
                  Rp. {{ Intl.NumberFormat().format(transaction.amount) }} &middot; {{ transaction.created_at }}
                </p>
              </div>
            </div>
          </div>
